import type { RootElement } from '@hamsterbase-third-party-internal/highlighter/src/types/index.js';
import {
  ID_DIVISION,
  DATASET_IDENTIFIER,
...
const findAncestorWrapperInRoot = (
  $node: HTMLElement,
  $root: RootElement
): HTMLElement => {
 ...
 */
export const getExtraHighlightId = (
  $node: HTMLElement,
  $root: RootElement
): string[] => {
  $node = findAncestorWrapperInRoot($node, $root);
  ...
  //@ts-ignore
  return $node.dataset[CAMEL_DATASET_IDENTIFIER_EXTRA].split(
    ID_DIVISION
  ).filter((i) => i);
};
...
export const forEach = (
  $nodes: NodeList,
  cb: (n: Node, idx: number, s: NodeList) => void
): void => {
