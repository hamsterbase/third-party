...
import type HighlightRange from '@hamsterbase-third-party-internal/highlighter/src/model/range/index.js';
import type {
  PainterOptions,
  HookMap,
...
} from '@hamsterbase-third-party-internal/highlighter/src/painter/dom.js';
import {
  getHighlightsByRoot,
  forEach,
  addClass,
...
import { initDefaultStylesheet } from '@hamsterbase-third-party-internal/highlighter/src/painter/style.js';
import {
  ID_DIVISION,
  eventEmitter,
  DATASET_IDENTIFIER,
...
export default class Painter {
  ...
  private get document(): Document {
    return this.options.contentWindow.document;
  }
  ...
  highlightRange(range: HighlightRange): HTMLElement[] {
    ...
    if (!hooks.Render.SelectedNodes.isEmpty()) {
      $selectedNodes =
        hooks.Render.SelectedNodes.call(range.id, $selectedNodes) || [];
    }
  ...
  removeHighlight(id: string): boolean {
    ...
    const $spans = this.document.querySelectorAll<HTMLElement>(
    ...
    $toRemove.forEach(($s) => {
      ...
      const $fr = this.document.createDocumentFragment();
    ...
    $idToUpdate.forEach(($s) => {
      ...
      const $overlapSpan = this.document.querySelector<HTMLElement>(
